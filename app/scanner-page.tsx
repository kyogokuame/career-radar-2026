...
export default function ScannerPage() {
  ...
  return <><SiteNav active="scanner"/><main className="scanner-page">
    <section className="scanner-hero"><div><span>JOB SCAN · CURATED OPPORTUNITIES</span><h1>先筛岗位结构，<br/><em>再决定是否进入雷达。</em></h1><p>职位池按“现在值得推进 / 有条件探索 / 长期目标”三层排序：第一层强调第一跳可行性，第二层保留一个待验证门槛，第三层只保留长期组合价值。每日扫描目标池约 60 条，其中“现在值得推进”约 20 条，其余按匹配度分配到后两层。</p></div><aside><b>{adopted.length}</b><span>已采用</span><b>{declined.length}</b><span>暂不采用</span><p>采用与否仅保存在当前浏览器。</p></aside></section>
